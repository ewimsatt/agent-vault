...
- Vault-created commits use an isolated Git index: they include only the vault files changed by that operation, preserve unrelated staged work, and run the repository pre-commit hook
- Vault-controlled identifiers are lexically constrained before filesystem access: agent and group names are single normal components; secret paths may be nested normal components. Traversal, absolute paths, separators in identifiers, backslashes, and control characters are rejected. Both SDKs reject malformed secret paths before pulling or reading files. Explicit key, input, and restore destination paths remain user-controlled paths.
