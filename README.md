...
- **The pre-commit hook is a guardrail.** Initialization installs a versioned wrapper in Git's effective hook path, including `core.hooksPath`. It scans staged index blobs for recognizable private-key markers before and after a preserved executable custom hook. Existing custom-hook bytes and executable permissions are kept in a sidecar; ambiguous legacy, symlink, non-regular, or conflicting hook states are refused rather than overwritten. This is not a general API-key scanner or an enforcement boundary: hooks are local, can be bypassed with Git's standard escape hatch, do not travel with a clone, and cannot defend against a malicious local hook or host.
...
- **Vault lifecycle commands preflight affected records.** Before `grant`, `revoke`, `remove-agent`, or `recover-agent` changes any ciphertext, Agent Vault decrypts and prepares every affected record in memory. A corrupt later record therefore stops the operation without rewriting earlier secret records or replacing a recovered agent key. This protects malformed-record failures before writing begins; it is not a crash-safe transaction for disk, permission, or Git failures that occur during the later write/commit phase.
